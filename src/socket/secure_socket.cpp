...
bool SecureSocket::open_server( const string port, const string privkey, const string pass ) {
  ...
  if ( (int)pass.size() > m_rsa_size - 3*EVP_MAX_KEY_LENGTH ) {
    Logger::error() << "passphrase too long! Max allowed length is " << m_rsa_size - 3*EVP_MAX_KEY_LENGTH << " characters " << endl;
    return false;
  }
  else {
    m_pass = pass;
...
bool SecureSocket::open_client( const string host, const string port, const string pubkey, const string pass ) {
  ...
  int rounds = 5;
  unsigned char nonce_a[ EVP_MAX_KEY_LENGTH ], salt[8];
  RAND_bytes( nonce_a, EVP_MAX_KEY_LENGTH );
  RAND_bytes( salt, 8 );

  int res = EVP_BytesToKey( CIPHER_ALG, EVP_sha1(), salt,
                            (unsigned char*) m_pass.data(),
                            m_pass.size(), rounds, m_session_key, nonce_a );
  if (res <= 0) {
  ...
  // M1 RECEIVE
  unsigned char nonce_b[ EVP_MAX_KEY_LENGTH ];
  int nonce_size = recv( (char*) nonce_b, EVP_MAX_KEY_LENGTH );
  if ( nonce_size != EVP_MAX_KEY_LENGTH ) {
  ...
  int message_size = 3*EVP_MAX_KEY_LENGTH + m_pass.size();

  memcpy( message+0*EVP_MAX_KEY_LENGTH, m_session_key, EVP_MAX_KEY_LENGTH );
  memcpy( message+1*EVP_MAX_KEY_LENGTH, nonce_a, EVP_MAX_KEY_LENGTH );
  memcpy( message+2*EVP_MAX_KEY_LENGTH, nonce_b, EVP_MAX_KEY_LENGTH );
  memcpy( message+3*EVP_MAX_KEY_LENGTH, m_pass.data(), m_pass.size() );
  ...
  if ( encrypted_size != m_rsa_size ) {
    Logger::error() << "cannot encrypt M2" << endl;
    return false;
  }
  ...
  if ( decrypted_size <= 0 ) {
    Logger::debug() << "cannot decrypt M3" << endl;
    return false;
  }
  if ( (decrypted_size != 2*EVP_MAX_KEY_LENGTH) ||
       memcmp( decrypted, m_session_key, EVP_MAX_KEY_LENGTH ) ||
       memcmp( decrypted+EVP_MAX_KEY_LENGTH, nonce_a, EVP_MAX_KEY_LENGTH ) ) {
    Logger::debug() << "wrong M3 received" << endl;
    delete[] decrypted;
    return false;
  }
...
bool SecureSocket::accept( SecureSocket& socket ) const {
  ...
  // M1 SEND
  unsigned char nonce_b[ EVP_MAX_KEY_LENGTH ];
  RAND_bytes( nonce_b, EVP_MAX_KEY_LENGTH );
  if ( ! socket.send( (char*) nonce_b, EVP_MAX_KEY_LENGTH ) ) {
    Logger::error() << "cannot send M1" << endl;
    return false;
  }
  ...
  if ( encrypted_size != m_rsa_size ) {
    Logger::error() << "cannot receive M2" << endl;
    return false;
  }
  ...
  if ( (decrypted_size != message_size) ||
       memcmp( decrypted+2*EVP_MAX_KEY_LENGTH, nonce_b, EVP_MAX_KEY_LENGTH ) ||
       memcmp( decrypted+3*EVP_MAX_KEY_LENGTH, m_pass.data(), m_pass.size() ) ) {
    Logger::error() << "error on M2 decryption or wrong passphrase" << endl;
    return false;
  }
  memcpy( socket.m_session_key, decrypted, EVP_MAX_KEY_LENGTH );

  // M3 SEND
  if ( ! socket.send_secure( decrypted, 2*EVP_MAX_KEY_LENGTH ) ) {
    Logger::error() << "cannot send M3" << endl;
    return false;
  }
